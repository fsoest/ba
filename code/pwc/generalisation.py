...
plt.scatter(N_arr[:10], -1 * E_uni_sum[:10], label='Unidir. LSTM')
plt.scatter(N_arr[:10], -1 * E_bi_sum[:10], label='Bidir. LSTM')
plt.scatter(N_arr[:10], -1 * E_triv_sum[:10], label='Local opt.')
plt.legend()
plt.xlabel('$N$')
plt.ylabel('$W$')
plt.savefig('/home/fsoest/ba/phystex/img/gen_dt_1.png')
