...
\end{align}
Here we use the partial Hamiltonian $H_S^i$ on the system at time step $i \in [1, N - 1]$, as well as corresponding system density matrix $\rho_S(i \Delta \mathrm{T})$.
...
\begin{align}
	H_S^i &= \frac{1}{2} \left[\sin(\theta_D^i) e^{i\phi_D^i} + \sin(\theta_T^i) e^{i\phi_T^i}\right] \sigma_{+} + h.c. =  H_{DS} + H_{ST}.
\end{align}
